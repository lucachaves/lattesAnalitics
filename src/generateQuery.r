...
generate.specific.select <- function(kindQuery){
	if(generate.query.kindQuery == 'name-lat-lon-source-target'){
		paste(
			"SELECT ",
			kindContext,
...
generate.specific.query <- function(kindContext, nacionalSource, instituitionSource, nacionalTarget, instituitionTarget, filter, kindQuery){
	specific.select <- generate.specific.select(kindQuery)
	specific.from <- paste(
		"FROM ",
		"public.edge edge",
	...
	if(kindFlow == 'fnf'){
		...
		if(global){
			union <- paste(
				...
				union <- generate.specific.query(kindContext,TRUE,TRUE,TRUE,TRUE,filte,kindQueryr)
			}
...
generate.query <- function(kindContext, kindFlow, kindQuery='name-lat-lon-source-target', kindTime='all', valueTime=NULL, global=TRUE,filter=c(), kindMoment='', flowMoment='target', range=c(1950,2013), limit=10){
...
sql.instituition.count.rank <-function(flowMoment, kindMoment, range,limit=limit){
...
query.filter <- function(kindFilter, range=NULL){
	valueFilter <- c()
	filterValue <- if(is.element('doutorado',kindFilter)){
  	c(valueFilter, "edge.kind='doutorado'")
  }else if(is.element('to-doutorado-USP',kindFilter)){
  	c(valueFilter, "edge.kind='doutorado' AND instituitionTarget.id = 179")
  }else if(is.element('from-doutorado-USP',kindFilter)){
  	c(valueFilter, "edge.kind='doutorado' AND instituitionSource.id = 179")
  }else if(is.element('to-doutorado-UFPE',kindFilter)){
  	c(valueFilter, "edge.kind='doutorado' AND instituitionTarget.id = 588")
  }else if(is.element('from-doutorado-UFPE',kindFilter)){
  	c(valueFilter, "edge.kind='doutorado' AND instituitionSource.id = 588")
  }else if(is.element('excludeAllBrazilFlows',kindFilter)){
  	c(valueFilter, "(countrySource.id != 131 AND countryTarget.id != 131)")
  }else if(is.element('excludeFlowsInBrazil',kindFilter)){
  	c(valueFilter, paste("edge.id NOT IN (",sql.inst.inst.nacional,") AND edge.id NOT IN (",sql.city.inst.nacional,")",sep=''))
  }else if(is.element('onlyFlowsInBrazil',kindFilter)){
  	c(valueFilter, paste("edge.id IN (",sql.inst.inst.nacional," UNION",sql.city.inst.nacional,")",sep=''))
  }else if(is.element('topDegreeInst',kindFilter)){
  	c(valueFilter, paste("instituitionSource.id IN (",sql.inst.top10.degree,") AND instituitionTarget.id IN (",sql.inst.top10.degree,")",sep=''))
  }else if(is.element('rangeYear',kindFilter)){
  	paste('edge.end_year IN (',paste(range,collapse=', '),') ',sep='')
  }
